...
addChild mem ptr childName child = 
    let
        newMem = case mem !! fromInteger ptr of
            FsDirectory name children parent -> replace ptr 
                (FsDirectory name (Map.insert childName child children) parent) mem
    in
        case mem !! fromInteger child of
...
addLsObject ptr mem obj@(FsFile name _) = 
    let 
        (child, newMem) = addFsObject mem obj
    in
        addChild newMem ptr name child
...
getFs (command:rest) mem ptr =
    case (command, mem !! fromInteger ptr) of
        ...
        (Cd dir, FsDirectory _ children _) ->
            if Map.member dir children then
                getFs rest mem $ fromJust $ Map.lookup dir children 
            else
                let
                    (newPtr, newMem) = addFsObject mem $ FsDirectory dir Map.empty 0 
                in
                    getFs rest (addChild newMem ptr dir newPtr) newPtr
...
problem7_1 :: [Command] -> Integer
problem7_1 commands = let mem = getFs commands [FsEmpty] 0 in countResult mem (mem !! 1)

problem7_2 :: [Command] -> Integer
problem7_2 commands = 
    let mem = getFs commands [FsEmpty] 0 in
        minimum $ filter (>= (getFsSize mem (mem !! 1) - 40000000)) $ map (getFsSize mem) $ getDirectories mem (mem !! 1)
...
parseLsObject text = 
    let 
        (size, ' ':name) = splitAt (fromJust $ ' ' `elemIndex` text) text
...
main = do
    handle <- openFile "input/input7.txt" ReadMode
    content <- hGetContents handle

    let 
        commandsText = splitOn (pack "\n$ ") (pack $ tail $ tail content)
        commands = map (parseCommand . unpack) commandsText
        in do
            print $ problem7_1 commands
            print $ problem7_2 commands
